...
(def I2C-REPLY               0x77) ;;I2C reply messages from an I/O board to a host, only for read/read-continuously
...
;; read-continuously indicates that the firmware should continuously
...
(defn- port-identifier
  "Given a port name return its identifier. (billwinkler version)"
  [port-name]
...
(defmethod enable-pin :firmata [conn type pin]
  (cond (= type :analog) (write-bytes conn (bit-or REPORT-ANALOG pin) 1)
        ...
        :else (throw (Exception. "Unknown pin type."))))
...
(defmethod i2c-blocking-read :firmata [conn slave-addr register bytes-to-read  & {:keys [timeout]}]
  ;; Note, Firmata protocol doesn't allows you to distinguish between
  ;; read replies and read-continuously reports, both are marked as I2C_REPLY
...
(defn- handle-sysex [conn in]
  (let [cmd (.read in)]
    (cond
      ...
      :else (read-to-sysex-end in) ;; discard
      )))
    ...
    (let [data (.read in)]
      (cond
        ...
        (= (bit-and data 0xF0) DIGITAL-MESSAGE) (let [port (bit-and data 0x0F)
                                                      [_lsb _msb val] (read-multibyte in)]
